...
def average_over_repetitions(n_repetitions, n_timesteps, learning_rate, gamma, action_selection_kwargs, replay_buffer_size=1000, target_net_delay=100, eval_interval=500):
...
def experiment(experiment_comment=None, overwrite=False):
    ...
    runs_kwargs = [
                ...
                dict(name="DQN_tn", target_net_delay=None),
                dict(name="DQN_rb", replay_buffer_size=None),
                dict(name="DQN_rb_tn"), #standard run, used in all further experiments 
                
                #runs to compare learning rate
                *[dict(name=f"DQN_lr_{lr}", learning_rate=lr) for lr in [0.005, 0.01]],
                
                # runs to compare action selection
                *[dict(name=f"DQN_as_{as_kwargs['policy']}", action_selection_kwargs=as_kwargs) for as_kwargs in new_as_kwargs]
                ]
